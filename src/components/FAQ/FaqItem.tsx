...
export const FaqItem: React.FC<FaqItemProps> = ({ item, onClick, isOpen }) => {
    const itemRef = useRef<HTMLParagraphElement>(null);
    return (
        <div onClick={() => onClick()} className={styles.faq_item}>
            <div className={styles.faq_question} style={isOpen ? { marginBottom: '24px' } : { marginBottom: '0' }}>
                <p>{item.question}</p>
                <button className={`${styles.faq_btn} ${isOpen ? styles.btn_active : ''}`}>
            ...
            <div
                ...
                style={
                    isOpen
                        ? {
                              height: itemRef.current?.scrollHeight ?? 0,
                              opacity: '1',
                          }
                        : { height: '0px', opacity: '0' }
                }
            >
                <p ref={itemRef}>{item.answer}</p>
            </div>
        </div>
    );
};
